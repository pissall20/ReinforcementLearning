...
# Now our output layer is the probability of going up or down. Let's make sure this output is between 0 and 1 by passing
...
def preprocess_observations(
    input_observation, prev_processed_observation, input_dimensions
):
...
def update_weights(weights, expectation_g_squared, g_dict, decay_rate, learning_rate):
    ...
    for layer_name in weights.keys():
        ...
        expectation_g_squared[layer_name] = (
            decay_rate * expectation_g_squared[layer_name] + (1 - decay_rate) * g ** 2
        )
...
def main():
    ...
    while True:
        env.render()
        processed_observations, prev_processed_observations = preprocess_observations(
            observation, prev_processed_observations, input_dimensions
        )
        ...
        if done:  # an episode finished
            ...
            if episode_number % batch_size == 0:
                update_weights(
                    weights, expectation_g_squared, g_dict, decay_rate, learning_rate
                )
            ...
            print("episode number: %d" % episode_number)
            print(
